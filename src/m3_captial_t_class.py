...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        # --------------------------------------------------------------
        hcenter = intersection_center
        hrectulc = rg.Point(hcenter.x - width / 2, hcenter.y - letter_thickness / 2)
        hrectlrc = rg.Point(hcenter.x + width / 2, hcenter.y + letter_thickness / 2)
        vrectulc = rg.Point(hcenter.x - letter_thickness / 2, hcenter.y - letter_thickness / 2)
        vrectlrc = rg.Point(vrectulc.x + letter_thickness, vrectulc.y + height)
        self.h_rect = rg.Rectangle(hrectulc, hrectlrc)
        self.v_rect = rg.Rectangle(vrectulc, vrectlrc)
